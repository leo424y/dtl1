...
class PostsController < ApplicationController
  ...
  def news
    if params[:start_date].present? || params[:end_date].present?
      @posts = Post.search_date(params, 'date')
      end
    if params[:description].present?
      @posts = @posts.search_context(params[:description], 'title')
      end
    respond_to do |format|
  ...
  def index
    ...
    if params[:start_date].present? || params[:end_date].present?
      @posts = @posts.search_date(params, 'date')
      end
    ...
    @posts_date = @posts.pluck(:updated).map { |x| Date.parse(x).strftime('%Y-%m-%d') }.group_by { |month| month }.map do |month, xs|
      [month,
       xs.count] # true
    end

    @posts_date_social = @posts.where(source: 'facebook').pluck(:updated).map { |x| Date.parse(x).strftime('%Y-%m-%d') }.group_by { |month| month }.map do |month, xs|
      [month,
       xs.count]   # true
    end

    @posts_date_news = @posts.where(source: 'news').pluck(:updated).map { |x| Date.parse(x).strftime('%Y-%m-%d') }.group_by { |month| month }.map do |month, xs|
      [month,
       xs.count]   # true
    end
    ...
    @post_tags = @posts.pluck(:archive).map do |x|
                   x['data'][0]['tags'] if x['data'] && x['data'][0]
                 end .flatten.each_with_object(Hash.new(0)) { |e, h| h[e] += 1; }.sort { |a1, a2| a2[1].to_i <=> a1[1].to_i }
